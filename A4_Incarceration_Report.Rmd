...
us_jail_pop_data <- us_jail_pop_data %>%
  mutate(population_density = total_jail_pop / total_pop)
  
merged_data <- merge(map_usa, jail_pop_data, by.x = "region", by.y = "state", all.x = TRUE)

ggplot() +
  geom_polygon(data = map_data("state"), aes(x = long, y = lat, group = group, fill = jail_pop_data$population_density)) +
  scale_fill_continuous(name = "Population Density", 
                         low = "lightblue", 
                         high = "darkblue", 
                         na.value = "grey50") +
  labs(
    title = "Jail Population Density by State",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal()
